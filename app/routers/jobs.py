...
async def scrape_jobs(
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail=f"{source} took too long to respond")
    except requests.exceptions.RequestException as e:
        raise HTTPException(status_code=502, detail=f"Failed to fetch jobs from {source}: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Failed to parse jobs from {source}: {str(e)}")
